...
setInterval(() => {
	...
	if (silentSyncFolders.length > 0) {
		...
		execScript(`rsync --relative --delete -avzP ${sources} ${distRoot}`);
	}
}, 2000);
...
var watcher = chokidar.watch(watchPaths, { persistent: true, ignoreInitial: true, ignored: ignoredPaths });
...
function execScript(script, sync) {
	if (sync == true) {
		try {
			execSync(script);
		} catch (error) {
