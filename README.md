...
Start by cloning all required git repositories in some local folder:

Open a console box (**Windows users should run a Git bash session!**) and clone the required git repositories in a local folder:
...
```bash
cd docs/build
sudo docker build -t ns-docs:1.0 .
```

### Building the documentation

Start the docker image created in the previous step:

```bash
sudo docker run -v /home/ubuntu/ns-docs:/root -p 9192:9192 -t ns-docs:1.0
```

Notice that we need to specify the full path to the `ns-docs` folder containing the dependent git repositories.

### Adding content

Edit an article in some of the repositories and open a web browser on your host machine and navigate to http://localhost:9192.
